...
export const BillCreatePage: React.FC = () => {
  ...
  // Extract edit bill ID from URL if editing /bills/:id/edit
  const editBillId = useMemo(() => {
    if (typeof window !== 'undefined') {
      const parts = window.location.pathname.split('/');
      if (parts[1] === 'bills' && parts[3] === 'edit' && parts[2]) {
        return parts[2];
      }
    }
    return null;
  ...
  const existingBill = useMemo(() => {
    ...
    return bills.find((b) => b.id === editBillId || (b as any)._id === editBillId) || null;
  }, [editBillId, bills]);

  const isEditMode = Boolean(editBillId && existingBill);

  const isEditLocked = useMemo(() => {
    if (!existingBill) return false;
    const hasPaid = (existingBill.paidAmount || 0) > 0;
    const hasMoved = existingBill.storeMovementStatus === 'PARTIALLY_MOVED' || existingBill.storeMovementStatus === 'FULLY_MOVED';
    return hasPaid || hasMoved;
  }, [existingBill]);

  // Selected Vendor
  const [selectedVendorId, setSelectedVendorId] = useState<string>(
    existingBill?.vendorId || vendors[0]?.id || ''
  );
  ...
  useEffect(() => {
    if (isEditMode && existingBill) {
      setSelectedVendorId(existingBill.vendorId || '');
      setFieldValue('vendorId', existingBill.vendorId || '');
      setFieldValue('vendorInvoiceNumber', existingBill.vendorInvoiceNumber || '');
      setFieldValue('billDate', existingBill.billDate || '');
      setFieldValue('dueDate', existingBill.dueDate || '');
      setFieldValue('shippingCharge', existingBill.shippingCharge || 0);
    ...
    } else if (creationMode === 'direct' && lineItems.length === 0 && approvedProducts.length > 0) {
  ...
  }, [creationMode, approvedProducts, isEditMode, existingBill]);
  ...
  const onFormSubmit = async (data: BillFormData) => {
    ...
    if (isEditLocked) {
      showAppToast('Bill cannot be edited: payments have been recorded or items moved to store.', 'error');
      return;
    }
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-slate-200 pb-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-bold text-slate-900">
                {isEditMode ? `Edit Vendor Bill: ${existingBill?.billNumber}` : 'Record New Vendor Bill'}
              </h1>
              {isEditLocked && (
                <span className="inline-flex items-center gap-1 text-[11px] font-bold text-rose-700 bg-rose-50 px-2 py-0.5 rounded border border-rose-200">
                  <Lock className="w-3 h-3 text-rose-600" />
                  Locked (Financial / Store Inward)
                </span>
              )}
            </div>
            <p className="text-xs text-slate-500">
              {isEditMode
                ? 'Modify bill particulars prior to payment disbursements or store inward movements'
        ...
        {isEditLocked && (
          <div className="p-4 rounded-xl border border-rose-200 bg-rose-50/80 flex items-start gap-3 text-xs text-rose-900 shadow-xs">
            ...
            <div>
              <h4 className="font-bold text-rose-900">Bill Immutability Enforced</h4>
              <p className="mt-0.5 text-rose-700">
                This bill cannot be modified because disbursements (₹
                {(existingBill?.paidAmount || 0).toLocaleString('en-IN')}) or store inventory movements ({existingBill?.storeMovementStatus}) have already been recorded. Indian accounting standards mandate audit immutability once ledger or stock entries are booked.
              </p>
            </div>
          </div>
        )}

        {/* Mode Selector Pill - hidden in edit mode */}
        {!isEditMode && (
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-xs space-y-4">
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-200">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-6 py-2.5 rounded-xl bg-blue-600 text-xs font-semibold text-white hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition shadow-xs cursor-pointer"
            >
              <CheckCircle2 className="h-4 w-4" />
              <span>{isEditMode ? 'Update Vendor Bill' : 'Confirm & Record Bill'}</span>
            </button>
          </div>
        </div>
      </form>
